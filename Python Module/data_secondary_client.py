...
class send_UR:
    ...
    def forward_kin(self, joint, theta, colum=0):
        '''
        Returns [nx ox ax x
                 ny oy ay y
                 nz oz az z
                 0  0  0  1] of joint 1 to 5 in relation to base using forward Kinematics
                 for joint 6 (end-effector) returns (x,y,z,rx,ry,rz)
        :param joint: int, number of joint wanted from 1 to 6, if 0 returns all
        :param theta: list of joint angles in rad, starting from joints 1 through 6 (given by get_joint_pos function)
        :param colum: column of the joint configure matrix "th" you would like to use
        :return:
        '''

        th = np.matrix([[theta[0]], [theta[1]], [theta[2]], [theta[3]], [theta[4]], [theta[5]]])

        A_1 = self.AH(1, th, colum)
        A_2 = self.AH(2, th, colum)
        A_3 = self.AH(3, th, colum)
        A_4 = self.AH(4, th, colum)
        A_5 = self.AH(5, th, colum)
        ...
        if joint == 1:
            ...
            T_06 = A_1 * A_2 * A_3 * A_4
        elif joint == 5:
            T_06 = A_1 * A_2 * A_3 * A_4 * A_5
        elif joint == 6:
            T_06 = self.get_tcp_pos()
        elif joint == 0:
            A_1 = np.delete(A_1, (3), axis=0)
            m1 = (A_1[:, 3].T).tolist()
            m2 = ((A_1*A_2)[:, 3].T).tolist()
            m3 = ((A_1*A_2*A_3)[:, 3].T).tolist()
            m4 = ((A_1*A_2*A_3*A_4)[:, 3].T).tolist()
            m5 = ((A_1 * A_2 * A_3 * A_4 * A_5)[:, 3].T).tolist()
            T_06 = [m1[0], m2[0], m3[0], m4[0], m5[0], self.get_tcp_pos()[0:3]]
        else:
            T_06 = 0
            print("The joint given is not possible. Choose from 0 to 6")
        return T_06
