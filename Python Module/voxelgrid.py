...
class VoxelGrid(Structure):
    ...
    def compute(self):
        """ABC API."""
        '''
        xyzmin = np.array([30, -150, -150])
        xyzmax = np.array([330, 150, 150])
        '''
        xyzmin = np.array([0, -300, -250])
        xyzmax = np.array([450, 170, 200])
        ...
        for i in range(3):
            # note the +1 in num
            s, step = np.linspace(xyzmin[i], xyzmax[i], num=(self.x_y_z[i] + 1), retstep=True)  #x_y_z evenly spaced samples in [xyzmin, xyzmax]
            segments.append(s)
            shape.append(step)
        self.segments = segments
        self.shape = shape

        self.n_voxels = self.x_y_z[0] * self.x_y_z[1] * self.x_y_z[2]       #number of voxels

        self.id = "V({},{},{})".format(self.x_y_z, self.sizes, self.regular_bounding_box)

        # find where each point lies in corresponding segmented axis/voxel
        ...
        self.voxel_n = np.ravel_multi_index([self.voxel_x, self.voxel_y, self.voxel_z], self.x_y_z) # make indexes 1D- each voxels of eaxh pixel
    ...
    def get_feature_vector(self, mode="binary", hvoxel=None, rvoxel=None, ovoxel=None, hjoints=None):
        ...
        if mode == "binary":
            vector[np.unique(self.voxel_n)] = 1       # just for print

        elif mode == "labeled":
            vector[np.unique(self.voxel_n)] = 10
            if rvoxel is not None:
                vector[rvoxel] = 100
                rv56 = rvoxel[4]
                rv12 = rvoxel[1]
                rv23 = rvoxel[2]
                ...
                j23 = []
                j45 = []
                n_vo = int(np.cbrt(self.n_voxels))
                while rv23 < rvoxel[1]-1:
                    rv23 = rv23+(n_vo*n_vo)
                    j23.append(rv23)
                while rv45 < rvoxel[3] - 1:
                    rv45 = rv45 + (n_vo * n_vo)
                    j45.append(rv45)
                while rv56 < rvoxel[5]-1:
                    rv56 = rv56+n_vo
                    j56.append(rv56)
                while rv12 < rvoxel[0]-1:
                    rv12 = rv12+n_vo
                    j12.append(rv12)
                vector[j56] = 100
                vector[j12] = 100
                vector[j23] = 100
                vector[j45] = 100
                vector[(rvoxel[3]+(n_vo * n_vo)):(rvoxel[2])] = 100
                #vector[rvoxel[5]+(6*n_vo)]=75
            if hvoxel is not None:
                vector[hvoxel] = 50
            if ovoxel is not None:
                vector[ovoxel] = 75
            #print(vector)
    ...
    def plot(self,
        ...
        if d == 2:
            ...
            fig, axes = plt.subplots(
                int(np.ceil(self.x_y_z[2] / 4)), 4, figsize=(15, 10))
            plt.tight_layout()
