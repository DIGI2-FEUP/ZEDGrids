...
void GLViewer::initialize() {
	...
	float span = 60.0f;
	float span2 = span / 10.0;
	for (float k = (int) 0; k <= (int)1; k+=5){
		float kk = k / 10.0;
	for (float i = (int) -span/2; i <= (int)span/2; i+=5) {
		float ii = i / 10.0;
		for (float j = (int)0; j <= (int)span; j += 5) {
			float jj = j / 10.0;
			// y planes
			grillvec.emplace_back();
			grillvec.back() = sl::float3(ii, kk, 0);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(ii, kk, span2);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(-span2/2, kk, jj);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(span2/2, kk, jj);
			}
		}
		...
		/*for (float j = (int)-span; j <= (int)span; j +=7) {
			float jj = j / 10.0;
			// y planes
			grillvec.emplace_back();
			grillvec.back() = sl::float3(ii, jj, -span2);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(ii, jj, span2);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(-span2, jj, ii);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(span2, jj, ii);
			// x planes
			grillvec.emplace_back();
			grillvec.back() = sl::float3(jj, ii, -span2);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(jj, ii, span2);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(jj, span2, ii);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(jj, -span2, ii);
			// z planes
			grillvec.emplace_back();
			grillvec.back() = sl::float3(-span2, ii, jj);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(span2, ii, jj);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(ii, span2, jj);
			grillvec.emplace_back();
			grillvec.back() = sl::float3(ii, -span2, jj);
		}*/
...
void GLViewer::render() {
	if (!ended_) {
		...
		glLoadIdentity();
		glClearColor(0.12, 0.12, 0.12, 0.5f);
		//glLineWidth(2);
		glPointSize(1);
		update();
		mtx_people.lock();
...
CameraGL::CameraGL(Translation position, Translation direction, Translation vertical) {
	...
	view_.setIdentity();
	updateView();
	setProjection(100, 100, 0.01f, 100.f);
	updateVPMatrix();
}
